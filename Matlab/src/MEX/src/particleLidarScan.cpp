...
void findRange(Map *map, Scanner *scanner, Pose *pose, double *range, double *xr, double *yr, double *C){
    //    mexErrMsgTxt("Why the Fuck is this crashing?");
    //    return;
        ...
        if (   (xs-x[0]>-eps) && (xs-x[N-1]<eps)
            ...
            do{
                ...
                if(z[yci + M*xci]>0){
                    ...
                    // mexErrMsgTxt("soms 237254");
                    // return;
                }
                // mexErrMsgTxt("sasdgjoiawheiufasdi54");
                // return;
                
            } while (!hit && ((xci != xei) || (yci != yei)));
        }
        // if(i==100){
        // mexErrMsgTxt("what the absolute fuck!?!?!?!");
        // return;}        
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    Eigen::MatrixXd ranges(scanner.numScans,M), xr(scanner.numScans,M), yr(scanner.numScans,M); 
    ranges.setZero();
    xr.setZero();
    yr.setZero();
    ...
    for(int i = 0; i < M; i++){
        double tempRange[scanner.numScans], tempXr[scanner.numScans], tempYr[scanner.numScans];
        tempPose.x = pose.x[i];
        tempPose.y = pose.y[i];
        tempPose.h = pose.h[i];
    //     //Now loop over the scans and find the range
        findRange(&map,&scanner,&tempPose,tempRange,tempXr,tempYr,C);
        Eigen::Map<Eigen::MatrixXd> rangeMap(tempRange,scanner.numScans,1);
        Eigen::Map<Eigen::MatrixXd> xrMap(tempXr,scanner.numScans,1);
        Eigen::Map<Eigen::MatrixXd> yrMap(tempYr,scanner.numScans,1);
        ranges.col(i) = rangeMap;
        xr.col(i) = xrMap;
        yr.col(i) = yrMap;
    }

    plhs[0] = mxCreateDoubleMatrix(scanner.numScans,M,mxREAL);
    Eigen::Map<Eigen::MatrixXd> rangeOut(mxGetPr(plhs[0]),scanner.numScans,M);
    rangeOut = ranges;

    plhs[1] = mxCreateDoubleMatrix(scanner.numScans,M,mxREAL);
    Eigen::Map<Eigen::MatrixXd> xrOut(mxGetPr(plhs[1]),scanner.numScans,M);
    xrOut = xr;

    plhs[2] = mxCreateDoubleMatrix(scanner.numScans,M,mxREAL);
    Eigen::Map<Eigen::MatrixXd> yrOut(mxGetPr(plhs[2]),scanner.numScans,M);
    yrOut = yr;

...
map.logodds = log(map.exp) - log1p(-map.exp); % Set log odds to -inf for empty and +inf for occupied to ensure true map drawn
...
% Define paramates of boat shape
L = 2*param.l;
W = 0.2;
space = param.d/2-W/2;
...
% Loop through the vector of time steps desired
for i = 1:length(tHist)-1
    try
        ...
        % Included for Legacy resons, old solutions delayed use of lidar for the first few time steps 
        if(tHist(i) > 0)
            %Simulate Lidar and calcualte likelihood
